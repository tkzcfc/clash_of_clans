...
# 针对Flags#number[][]这种类型返回正确的名称Flags
def splitKey(value):
    return value.split("#")[0]

# 针对Flags#number[][]这种类型返回正确的名称number[][]
def splitType(name, type):
    var = name.split("#")
    if len(var) == 2:
        return var[1]
    return type
...
def exportToTS(luaFile, xlsxFileName, rootDir, ignoreFirstColumn, ignoreTypeMark, outFileArr):
    ...
    for sheetInfo in sheetInfoArr:
        ...
        for idx in xrange(0, colCount):
            curName = splitKey(rowInfo[idx]['name'])
            curType = rowInfo[idx]['type']
            if curType != VarType.NULL:
                curType = fmtTsType(rowInfo[idx]['type'])
                curType = splitType(rowInfo[idx]['name'], curType)
                headerScript = headerScript + indent(1) + fmtKey(curName, ignoreTypeMark, False) + ": " + curType + ",\n"
