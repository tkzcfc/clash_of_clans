...
# 导出
def exportToJson(luaFile, xlsxFileName, rootDir, ignoreFirstColumn, ignoreTypeMark, outFileArr):
        ...
        for rowData in sheetInfo['rowDataArr']:
            ...
            for idx in xrange(0, colCount):
                ...
                if curType != VarType.NULL:
                    if isFirstEle:
                        isFirstEle = False
                        rowScript = rowScript + '{}"{}": {{\n'.format(indent(1), fmtLuaKey(tmpValue, ignoreTypeMark, False))
                        tabKeysArr.append(fmtLuaKey(tmpValue, ignoreTypeMark, False))
                        if not ignoreFirstColumn:
                            rowScript = rowScript + '{}"{}": {}, \n'.format(indent(2), fmtLuaKey(curName, ignoreTypeMark, False), tmpValue)
                    else:
                        rowScript = rowScript + '{}"{}": {}, \n'.format(indent(2), fmtLuaKey(curName, ignoreTypeMark, False), tmpValue)
            scriptLineArr.append(rowScript.rstrip(', \n') + '\n' + indent(1) + '}')
...
def exportDir(workDir, outDir):
            ...
            for ext in ExtensionArr:
                if f.endswith(ext) and not f.startswith("~$"):
                    ...
                    if not os.path.isdir(out):
                        os.makedirs(out)
                    if not exportToJson(fullPath, os.path.join(root, f), workDir, False, True, outFileArr):
                        log("发布失败<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
                        return False


    script = 'export namespace GameCfgKey {' + row_separator
    ...
    for file in outFileArr:
        ...
        baseKey = baseKey.replace('\\', '_')
        script = script + '    export const {0}: string = \"{0}\";'.format(baseKey) + row_separator
    script = script + '}' + row_separator
    
    output_file = codecs.open(outDir + 'GameCfgKey.ts', 'w', output_encoding)
    output_file.write(script)
    output_file.close()
